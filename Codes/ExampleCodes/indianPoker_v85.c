...
int BETTING_START(int com_card){
      ...
      // tactswitch 베팅 입력
	  	int user_answer = tactsw_get_with_timer(10); close(dot_mtx);
...
// void WIN_LED(int user_score, int com_score) {
//   // 0xFE, 
//     char led_array[] = { 0xFC, 0xF8, 0xF0, 0xE0, 0xC0, 0x80, 0x00 };   // 16진수 값들 미리 선언
//     unsigned char data;
...
int start(int* cards1, int* cards2){
  ...
  for(i = 0; i < ROUND; i++){
    ...
    if(win_lose(user_answer, correct_answer)){           // 베팅 결과 확인
        ...
        write(leds, &data, sizeof(unsigned char)); usleep(2000000);
    ...
    PRINT("   CHECK  THE      USER  CARD   ");  usleep(2000000);
