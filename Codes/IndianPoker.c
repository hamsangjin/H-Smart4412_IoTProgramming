...
// 함수 선언부 
// 베팅할 때 호출하는 함수로 컴퓨터 카드와 해당 베팅 라운드, 유저 카드들을 입력받는 역할(힌트 출력 위해) 
      ...
      if(1<=b && b<=12){
        switch (b){
					...
					default: {
						...
						//print("press other key"); usleep(5000000); 			// 구현할 수 있으면 해보기
					}	             
				}	
			}
		}
...
//1초마다 fnd에 남은 제한시간을 출력 //반환값 0: 입력없음 1~128: 입력값 
int dipsw_get_with_timer(int t_second) 
{   
	int selected_dip = 0;
	unsigned char d=0;
